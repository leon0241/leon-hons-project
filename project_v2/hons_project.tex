...
\abstract{
   A popular technique for representing processes in Concurrency Theory is categorisation into models called Process Algebras. Many Process Algebras exist, and efforts have been made to contextualise different Algebras against each other to measure notions of which Algebras are ``better''.

   In this paper, we use the notion of ``expressiveness'', namely, ``Can one Algebra express more tasks than another'', and I present a translation between two popular Algebras, $\acp$ and $\csp$, showing that $\acp$ with Functional Renaming is at least as expressive as $\csp$ up to rooted branching bisimulation.
}
