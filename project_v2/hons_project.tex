...
\begin{dfn}[Triggering in ACP]{dfn:acp-triggering}{}
   \[\Gamma(P) := \rho_{f_{\mathtt{trig}}}[\partial_{H_{1}}(P | | \mathtt{first}(\mathtt{next}^{\infty}))])\]
...
\begin{enumerate}[label=\alph*)]
   ...
   \item Apply $\rho_{f_{\mathtt{trig}}}$ to equation \ref{eq:fnn}. Via \ref{dfn:f1}, the final result is
...
For our encoding, we take $\mathscr{H} = \{\mathtt{first}, \mathtt{next}, \mathtt{origin}, \mathtt{split}\}$, and therefore $H_{1} = A_{0} \uplus \{\mathtt{first}, \mathtt{next}, \mathtt{origin}, \mathtt{split}\}$ as defined in \ref{dfn:sets}. We then modify Definition \ref{dfn:comm-triggering} to include additional communications for the operators $\mathtt{left}$, $\mathtt{origin}$, and $\mathtt{split}$
...
\begin{dfn}[Left-Tagging]{dfn:lefter}{}
   We define a renaming operator $f_{\mathtt{origin}} : A \to A$ to turn a trace of a process $P$, $a.b.c$ into the trace
   \[a_{\mathtt{origin}}.b_{\mathtt{origin}}.c_{\mathtt{origin}}\]
   and is defined as
   \[f_{\mathtt{origin}}(a) = a_{\mathtt{origin}}\]
\end{dfn}
...
\[\sigma = \langle X \mid X = \mathtt{origin}.X + \mathtt{split} \rangle\]
...
\[\mathscr{T}(P \triangle Q) = \partial_{H_{0}}\Bigl(\rho_{f_{\mathtt{post}}}\Bigl[
	 \bigl(\rho_{f_{\mathtt{origin}}}(\mathscr{T}(P)) | | \sigma\bigr) | | \Gamma(\mathscr{T}(Q))
\Bigr]\Bigr)\]
...
, and therefore $H_{1} = A_{0} \uplus \{\mathtt{first}, \mathtt{next}, \mathtt{origin}, \mathtt{split}\}$ as defined in \ref{dfn:sets}. We then modify Definition \ref{dfn:comm-triggering} to include additional communications for the operators $\mathtt{origin}$, and $\mathtt{split}$

\begin{dfn}[Interrupt Separating]{dfn:interrupt-sep}{}
   We define a renaming operator $f_{\mathtt{split}} : A \to A$ to turn a trace of a process $P$, $a.b.c$ into the trace
   \[a_{\mathtt{origin}}.b_{\mathtt{origin}}.c_{\mathtt{origin}}\]
   and is defined as
   \[f_{\mathtt{origin}}(a) = a_{\mathtt{origin}}\]
\end{dfn}
...
\[\mathscr{T}(P \Theta_{A} Q) = \partial_{H_{0}}\Bigl(\rho_{f_{\mathtt{post}}}\Bigl[
	 \rho_{f_{\mathtt{origin}}}(\mathscr{T}(P)) | | \sigma.\mathscr{T}(Q)
\Bigr]\Bigr)\]
