...
Recall from \ref{dfn:helper-functions} and \ref{dfn:communications} the list of helper functions and communications:
...
\begin{dfn}[Translation of CSP to ACP]{dfn:trans}{}
   ...
   \trans{P \extchoice Q} &= \partial_{H_{0}}\Bigl(\fdef{post}\Bigl[\Gamma[\trans{P}]\, \pcomp \,\mathtt{choose}\, \pcomp \,\Gamma[\trans{Q}]\Bigl]\Bigr) \\
      \trans{P \pcomp_{A} Q} &= \partial_{H_{0}}\Bigl(\fdef{post}\Bigl[\fdef{syn}(\trans{P}) \pcomp \fdef{syn}(\trans{Q})\Bigr]\Bigr) \\
      \trans{P \backslash A} &= \partial_{A}{\trans{P}} \\
   % \trans{\mu p.P} &= \langle X \mid X = \tau.X \rangle
   \trans{f(P)} &= f(\trans{P}) \\
      \trans{P \triangle Q} &= \partial_{H_{0}}\Bigl(\fdef{post}\Bigl[ \bigl((\fdef{origin}\trans{P}) \pcomp \Pi\bigr) \pcomp \Gamma(\trans{Q}) \Bigr]\Bigr) \\
