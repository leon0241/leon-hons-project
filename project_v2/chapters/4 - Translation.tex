...
Working in the language $\acpt$ with the extension of Functional Renaming (written $ACP^{\tau}_{F}$), we first recall that $\acptf$ is paramaterised by a set of actions $\Sigma$ as defined in \ref{sc:acp}. We then start by defining some subsets of $\Sigma$ which we will use in our encodings.
...
\begin{dfn-s}[Trigger Function]{dfn:f1}{}
Define functions $f_{\mathtt{trig}}: \Sigma \to \Sigma$ where
\begin{align*}
...
\begin{dfn}[Postfix function]{dfn:postfix}{}
	Let $f_{\mathrm{post}} : \Sigma \to \Sigma$ where
	\vspace{-5pt}
	\[f_{\mathrm{post}}(\alpha) = \begin{cases}
			a      & \text{ if } \alpha = \alpha_{\mathrm{post}} \\
			\alpha & \text{otherwise}
		\end{cases}\]
\end{dfn}
...
\begin{dfn}[Helper Functions]{dfn:helper-functions}{}
	...
	\vspace{-8pt}
	\[f_{\mathrm{trig}}(\alpha) = \begin{cases}
			a_{\mathtt{first}} & \text{ if } \alpha = a_{\mathtt{ini}} \\
			\alpha             & \text{otherwise}
		\end{cases} \qquad f_{\mathrm{post}}(\alpha) = \begin{cases}
			a      & \text{ if } \alpha = \alpha_{\mathrm{post}} \\
			\alpha & \text{otherwise}
		\end{cases}\]
	We define functions for the remaining operators below. We use the notation $A_{T}$ to signify a target set, as used in \ref{ssec:parallel-composition}, \ref{ssec:interrupt}, and $\lambda$ to signify actions in $A$

	\begin{enumerate}
		\item $\fdef{syn} : \Sigma \to \Sigma$ is a function that renames any actions in the target set $A$. This is used in the translation of Parallel Composition (\ref{ssec:parallel-composition})
		      \vspace{-5pt}
		      \[
		      ...
		      \]
		      % \vspace{-30pt}
		\item $\fdef{origin} : \Sigma \to \Sigma$ is a function that renames actions in a process for use in operators. This is used in the translation of the Interrupt operator (\ref{ssec:stopping})
		      \vspace{-5pt}
		...
		\item $\fdef{split} : A \to A$ is a function that renames actions in a process for use in operators, and also renames actions in the target set $A_{T}$. This is used in the translation of the Interrupt operator (\ref{ssec:interrupt})
		      ...
		      \[
			      f_{\mathrm{split}}(a) = \begin{cases}
				      a_{\mathtt{split}} & \text{ if } a\in A \\
				      a_{\mathtt{origin}}           & \text{otherwise}
			      \end{cases}
		      \]
	\end{enumerate}
\end{dfn}
...
\subsection{Interrupt}\label{ssec:stopping}

The Interrupt operator $\triangle$ is defined with the following rules:
...
\begin{dfn}[Functions and Communications - Interrupt]{dfn:comms-stopping}{}
	As defined in \ref{dfn:helper-functions} and \ref{dfn:communications}, the following communications and functions are defined over External Choice:
	\[f_{\mathtt{trig}}(a_{\mathtt{first}}) = a_{\mathtt{ini}} \qquad
		f_{\mathtt{trig}}(a_{\mathtt{next}}) = a  \qquad
		f_{\mathrm{post}}(a_{\mathtt{post}}) = a \qquad f_{\mathrm{post}}( \alpha) = \alpha
...
We can now define an encoding of the $\csp$ Interrupt operator $\triangle$ in $\acptf$. We start off with a new process, which we will call $\Pi$. This is defined as the process
...
\subsection{Throw}\label{ssec:interrupt}

The Throw operator $\interrupt$ is defined with the following rules:
...
\begin{dfn}[Functions and Communications - Throw]{dfn:comms-interrupt}{}
	As defined in \ref{dfn:helper-functions} and \ref{dfn:communications}, the following communications and functions are defined over External Choice:
...
We can now define an encoding of the $\csp$ Throw operator $\interrupt$ in $\acptf$. We employ the use of the same operator as in the Interrupt operator, $\Pi$ which is defined as
\[\Pi = \langle X \mid X = \mathtt{origin}.X + \mathtt{split} \rangle\]
Or, visualised as a process graph:
