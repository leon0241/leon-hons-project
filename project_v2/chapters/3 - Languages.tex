...
\end{figure}
In Trace Semantics, the process $P$ is equivalent to the process $Q := a . b + a . c$, since they both have the traces $\{a . b,\,a . c\}$. However, this is clearly not the case in Bisimulation Semantics, where rule $2$ in Definition \ref{dfn:strong-bisim} is not satisfied, as there is no state in $Q$ that can perform both a $b$ action, and a $c$ action. Clearly, this implies that \textit{Bisimulation Equivalence} is a \textbf{finer} relation than \textit{Completed Trace Equivalence}. Here, \textit{finer} means that one equivalence can distinguish between more processes than another. We will also use \textbf{coarser} to mean the opposite.
