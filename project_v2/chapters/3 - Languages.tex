...
\begin{dfn}[\texorpdfstring{$\acpt$}{acpt}]{dfn:acpt}{}
   The process algebra $\acpt$ as described in \cite{bergstraACPtUniversalAxiom1989} is parameterised by a set of actions, $\Sigma$, and a communication function $|$. The grammar of $\acpt$ can be described with the following operations:
   \begin{align*}
...
\end{dfn}
We have $H, I \subseteq \Sigma$, and additionally we define the set $\Sigma_{\tau}$ to be the set $\Sigma \cup \{\tau\}$, as the silent step is not included in $\Sigma$. The operations can be described in the following manner:
...
\end{tikzcd}\]
    \caption{Example of Branching Bisimilarity. The process on the left ($a.\tau$ compared to $a$) is Rooted Branching Bisimilar, and the process on the right ($\tau.a$ compared to $a$) is only Branching Bisimilar. A blue dotted line indicates a relation between states}
