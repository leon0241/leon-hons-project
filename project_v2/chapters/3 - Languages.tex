...
We should also specify that $\trans{a} = a$ for each $a\in A$ specifies that any actions are consistent between algebras.
...
\begin{dfn}[Rooted Branching Bisimilarity]{dfn:rooted-branching}{}
	...
	\begin{enumerate}
		...
		\item If $s \prightarrow{\alpha} s'$ for $\alpha\in \Sigma_{\tau}$ is an edge in $P$, and $s R t$, then either
		      ...
		      \end{enumerate}
		\item If $s \checkmark$ and $s R t$ then there exists a path $t \Rightarrow t'$ in $Q$ to a node $t'$ with $t'\checkmark$ and $s R t'$\footnote{As explained in Section \ref{ssec:termination}, this rule can be omitted without loss in function over the translation.}
