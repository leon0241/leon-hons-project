...
\begin{proof}
	...
	\begin{align*}
		...
		{\color{blue}\text{Applying CM3 and CM7}}&= \partial_{H_{0}}\bigl( \alpha.(P' \merge Q) + \phi.(Q' \leftmerge P) + (\alpha \mid \phi)(P' \merge Q')\bigr) \\
		{\color{blue}\text{Expanding $\partial_{H_{0}}$}}&= \partial_{H_{0}}(\alpha.(P' \merge Q)) + \partial_{H_{0}}(\phi.(Q' \leftmerge P)) + \partial_{H_{0}}\bigl((\alpha \mid \phi)(P' \merge Q')\bigr) \\
									    {\color{blue}\text{Applying $\partial_{H_{0}}$}}&= \partial_{H_{0}}(\alpha.(P' \merge Q)) + {\color{red}\delta.(Q' \leftmerge P)} + {\color{red}\delta.(P' \merge Q')} \\
...
\begin{lma}[]{lma:merge-simplification-merge}{}
	...
	Also note that an $c$ action can be made on the resulting process.
	\longrule{0.08ex}
...
\begin{proof}
	...
	Let $P, Q \in \csp$ be two processes. We want to show that $P \extchoice Q \rbrb \trans{P \extchoice Q}$. i.e.: we want to show that any move will result in a process that satisfies RBB. We show this by looking at individual traces and exhausting all possible moves that $P \extchoice Q$ can take, and confirm that $\trans{P \extchoice Q}$ can also take them, up to Rooted Branching Bisimilarity.
	...
	Now working in $\acptf$, we want to show that the translation is valid up to RBB, i.e. $\trans{P'} \rbrb \trans{P}$, or in general, $\trans{\mathcal{P}} \rbrb \mathcal{P}$ for any $\mathcal{P}\in \csp$

	From the definition of the Triggering operator \ref{oper:triggering}, we can derive the following equation:
	\[\Gamma(\trans{P}) = a_{\mathtt{ini}}.\trans{P'}\]
Therefore, since $\mathtt{choose}\in H_{0}$, and $a_{\mathtt{ini}} \mid \mathtt{choose} = a$\footnote{The communication technically becomes $a_{\mathtt{post}}$, but we will go back to $a$ to simplify the intuition}, we can derive the following process from Lemma \ref{lma:merge-simplification-merge}:
	\begin{align*}
		\trans{P \extchoice Q} = & \, \partial_{H_{0}}\Bigl(\fdef{post}\Bigl[\Gamma[\trans{P}]\merge \mathtt{choose}\merge \Gamma[\trans{Q}]\Bigl]\Bigr) \\
...
\begin{itemize}[leftmargin=*]
    \item The \textbf{Parallel Composition} operator $\pcomp$ is defined as the equation
		\[\trans{P \pcomp Q}     = \partial_{H_{0}}\Bigl(\fdef{post}\Bigl[\fdef{syn}(\trans{P}) \merge \fdef{syn}(\trans{Q})\Bigr]\Bigr)\]
		\begin{itemize}
		    \item Let $P'$ such that $P \prightarrow{\alpha} P'$, and $\alpha\not\in A$ for the target set $A$. $\fdef{syn}$ will not affect $P$. Since $q\in H_{0}\,\forall q\in Q$ and $\alpha\mid a_{\mathtt{syn}} = \delta$, we apply Lemma \ref{lma:merge-simplification-a}:
				\begin{align*}
					\trans{P \pcomp Q} =& \partial_{H_{0}}\Bigl(\fdef{post}\Bigl[\fdef{syn}(\trans{P}) \merge \fdef{syn}(\trans{Q})\Bigr]\Bigr) \\
	...
	\item The \textbf{Throw} operator is defined as the equation:
		...
		\end{itemize}
    \item The \textbf{Sliding Choice} operator $\sliding$ is defined as the equation
		...
		\begin{itemize}
		    \item Let $P'$ such that $P \prightarrow{\alpha} P'$. Since $\mathtt{choose}\in H_{0}$, and $\mathtt{choose} \mid a_{\mathtt{ini}} = a$, we apply Lemma \ref{lma:merge-simplification-merge}:
				...
				\end{align*}
				Nothing in $\trans{P'}$ can communicate with the action $\mathtt{shift}_{\mathtt{ini}}$, therefore the process is Strongly Bisimilar to $P' \sliding Q$.
...
From these proofs, we have shown that every translation of the $\csp$ operators are valid up to Rooted Branching Bisimularity.
