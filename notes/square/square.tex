...
\title{Encoding of CSP Square operator to ACP}
\author{Leon Lee}
...
\begin{dfn}[Sets of A]{dfn:sets}{}
	\begin{itemize}
	    ...
	    \item $A_{0}$ is the set of actions that actually get used in processes
	    \item $H_{0} = A - A_{0}$ is the set of working space operators or any other action that doesn't get used
	    \item $H_{1} = A_{0} \cup \{\mathrm{first}, \mathrm{second}, \mathrm{stop}, \mathrm{skip}, \mathrm{next}\}$ is the set of actions, plus some working operators
	\end{itemize}
\end{dfn}
...
\begin{dfn}[F1]{dfn:f1}{}
	Define a function $f_{1}: A_{0} \to H_{0}$ where
	\begin{align*}
		f_{1}(a_{\mathrm{first}}) &= a_{\mathrm{first}} \\
		f_{1}(a_{\mathrm{second}}) &= a_{\mathrm{second}} \\
...
\begin{dfn}[Communication]{dfn:communication}{}
	...
	\begin{align*}
		...
		\tau | \mathrm{first} &= \tau \\
		a | \mathrm{second} &= a_{\mathrm{second}}\\
...
\begin{dfn}[Triggering in ACP]{dfn:acp-triggering}{}
	\[\phi_{p} := \rho_{f_{1}}[\partial_{H_{1}}(p | | \mathrm{first}(\mathrm{next}^{\infty}))]\]
	is a function that turns a process $p = p_{1}.p_{2}.p_{3}.\dots$ into
	\[p = p_{\mathrm{first}}. p_{2}. p_{3}. p_{4}.\dots\]

	\longrule{0.08ex}

	\[\phi_{p} := \rho_{f_{1}}[\partial_{H_{1}}(p | | \mathrm{first}(\mathrm{second}(\mathrm{next}^{\infty})))]\]
	is a function that turns a process $p = p_{1}.p_{2}.p_{3}.\dots$ into
	\[p = p_{\mathrm{first}}. p_{\mathrm{second}}. p_{3}. p_{4}.\dots\]
\end{dfn}
